...
interface CropManagementLandPreparationProps {
  profile: CropManagementProfile;
}

export default function CropManagementLandPreparation({ profile }: CropManagementLandPreparationProps) {
  return (
    <section className="space-y-3 rounded-2xl border border-white/10 bg-slate-900/60 p-6">
      <h2 className="text-xl font-bold text-white">Land Preparation</h2>
      <ul className="space-y-2 text-sm text-slate-300">
        {profile.landPreparation.map((item) => (
