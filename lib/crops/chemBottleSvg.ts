...
/** Product-shot SVG: white bottle, farm field, technical name printed on the label. */
export function renderChemBottleSvg(technical: string, category?: ChemBottleCategory): string {
  ...
  const formSize = (formulation?.length ?? 0) > 14 ? 8 : 10;
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 400" width="240" height="400">
  ...
  ${names}
  ${formBlock}
  <text x="120" y="${formulation ? 252 : 232}" text-anchor="middle" fill="#111827" font-family="Arial, Helvetica, sans-serif" font-size="8" font-weight="800" letter-spacing="1.2">${KIND_LABEL[kind]}</text>
...
export function chemBottleImageSrc(technical: string): string {
  ...
  if (hit?.slug) return `/images/chem/bottles/${hit.slug}.svg`;
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(renderChemBottleSvg(technical))}`;
}
